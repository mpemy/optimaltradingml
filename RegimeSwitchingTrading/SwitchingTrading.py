...
#from SwitchingTradingModel10312023 import SwitchingTradingEnv

from SwitchingTradingModelPemy12152023 import SwitchingTradingEnv

#from SwitchingTradingModelPemy12112023 import SwitchingTradingEnv
#env= gym.make("LunarLander-v2")

env = SwitchingTradingEnv()
...
class agent():
    ...
    def act(self,state):
        ...
        shape = len(state)
        if shape == 2: # with shape = np.shape(state), then here would be shape[0] == 2
        ...
        action = dist.sample() # action is a probability distribution
        realAct=int(action.numpy()[0])
        realAct = 11 if realAct >11 else realAct
        realAct = 0 if realAct <0 else realAct
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        #shape = np.shape(state)
        shape = len(shape)
        if shape == 2: #shape[0] == 2
          data = tf.convert_to_tensor([state[0]], dtype= np.float32)
        else:
          data = tf.convert_to_tensor([state], dtype= np.float32)
        state0 = data
        state = np.array([state[0]]) # state = np.array([state])
...
for s in range(steps):
  ...
  while step<total:
    ...
    if step>=total:
      ...
      print("total reward after {s} steps is {}",s, total_reward)
      rk = nber
      env.newplotter(rk)
      #env.switchplotter(rk)
