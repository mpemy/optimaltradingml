...
env_week2 = SwitchingTradingTrainingEnv()
#env = TradingEnv()

low = env_week2.observation_space.low
high = env_week2.observation_space.high
...
for s in range(steps):
  ...
  while step<total:
    ...
    next_state, reward, done, _ = env_week2.step(action, nber)
...
testingEnv_week2 = SwitchingTradingTestingEnv() 
agentoo7 = agent()
steps = 6
total = 200 # batch size
nber=0
for s in range(steps):
  ...
  state = testingEnv_week2.reset(s)
  total_reward = 0
  all_aloss = []
  all_closs = []
  step = 0
  while step<total:
    ...
    nber = temp % 1523
    #ans =  env.step(action, nber)
    next_state, reward, done, _ = testingEnv_week2.stepTesting(action, nber)
    aloss, closs = agentoo7.learn(state, action, reward, next_state, done)
    all_aloss.append(aloss)
    all_closs.append(closs)
    state = next_state
    ...
    if step>=200:
      ...
      print("total reward after {} steps is {}",s, total_reward)
      rk = nber
      testingEnv_week2.newplotter(rk)
