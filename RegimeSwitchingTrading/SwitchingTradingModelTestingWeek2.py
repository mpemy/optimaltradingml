...
resultgspc = [dfGSPC31] # dfGSPC4 is a labor day, so it is empty
resultaapl = [dfAAPL31]
resultgoog = [dfGOOG31]
resultT = [dfT31]
resultvz = [dfVZ31]
...
def RewardReal(i, k, a, s):    
    ...
    return (DiscountFactor ** k) * (TraderVWAPReal(i, k, a, s) - MarketVWAP(i, k+1)) # tune the market vwap by increase it by 5%
...
class SwitchingTradingTestingEnv(gym.Env):
    ...
    def newplotter(self, episode): 
        ...
        for i in range(0,6):
            ...
            tVWAP = []
            if i==0 :
                name ='AAPL when the market(SP500) is up'
            elif i== 1:
                name = 'GOOG when the market(SP500) is up'
            elif i== 2: 
                name = 'IBM when the market(SP500) is up'
            elif i==3:
                name = 'AT&T when the market(SP500) is up'
            elif i==4:
                name = 'VZ when the market(SP500) is up'
            elif i== 5:
                name ='Exxon Mobil when the market(SP500) is up'
    ...
    def newgraph(trades, traderVWAP, MarketVWAP, stockLabel):
        ...
        Times = []
        date_and_time = datetime.datetime(2017, 10, 31, 9, 30, 0)
